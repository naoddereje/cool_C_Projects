...
// If balancing required, perform necessary rotations.
{
	...
	if(bf < -1){
		if(getBalanceFactor(root->rightChild)>0){
			printf("Node %d is unbalanced. Left of Right:\n", root->key);
			Node* temp;
			root->rightChild = rotateRight(root->rightChild);
			root = rotateLeft(root);
		}else{
			printf("Node %d is unbalanced. Right of Right:\n", root->key);
			root = rotateLeft(root);
		}
	}else if (bf > 1){
		if(getBalanceFactor(root->leftChild)<0){
			printf("Node %d is unbalanced. Right of Left:\n", root->key);
			Node* temp;
			root->leftChild = rotateLeft(root->leftChild);
			root = rotateRight(root);
		}else{
			printf("Node %d is unbalanced. Left of Left:\n", root->key);
			root = rotateRight(root);
		}
	}
	return root;
